...
export default function CTA() {
  ...
  const opacityIn = useTransform(scrollYProgress, [0, 0.25], [0, 1])

  return (
    <section ref={ref} className="relative w-full bg-[#0a0a0a] overflow-hidden">
      <motion.div style={{ scale: scaleIn, opacity: opacityIn }} className="relative min-h-[80vh] py-32 px-6 flex items-center justify-center">
...
<motion.h2 initial={{opacity:0,y:50,scale:0.95}} whileInView={{opacity:1,y:0,scale:1}} transition={{duration:1.2,delay:0.2,ease:[0.16,1,0.3,1]}} viewport={{once:true,margin:"-100px"}} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-light text-white/90 tracking-tight leading-tight mb-8">
            Siap Upgrade<br/><span className="bg-gradient-to-r from-cyan-200 to-white bg-clip-text text-transparent">Akun TikTok Lo?</span>
          </motion.h2>
          <motion.p initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} transition={{duration:0.8,delay:0.4,ease:[0.16,1,0.3,1]}} viewport={{once:true,margin:"-100px"}} className="text-sm sm:text-base text-white/30 font-light leading-relaxed max-w-xl mx-auto mb-12">
